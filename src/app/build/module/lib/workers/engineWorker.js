...
export function engine() {
    ...
    function loadLibrary(deviceId, libraryLocation, locationPath, preload, newDelayedRegistration, newHighEndBlurryRecognition, newLicenseKey, deviceModelName) {
        ...
        Module = {
            ...
            instantiateWasm: (importObject, successCallback) => {
                ...
                // istanbul ignore if
                if (wasmJSVersion !== "5.6.0") {
                    console.error(`The Scandit SDK Engine library JS file found at ${jsURI} seems invalid: ` +
                        `expected version doesn't match (received: ${wasmJSVersion}, expected: ${"5.6.0"}). ` +
    ...
    function getLibraryLocationURIs(libraryLocation) {
        ...
        if (/^https?:\/\/([^\/.]*\.)*cdn.jsdelivr.net\//.test(libraryLocation)) {
            libraryLocation = "https://cdn.jsdelivr.net/npm/scandit-sdk@5.6.0/build/";
            cdnURI = true;
        }
        else if (/^https?:\/\/([^\/.]*\.)*unpkg.com\//.test(libraryLocation)) {
            libraryLocation = "https://unpkg.com/scandit-sdk@5.6.0/build/";
            cdnURI = true;
        }
        if (cdnURI) {
            return {
        ...
        return {
            jsURI: `${libraryLocation}scandit-engine-sdk.min.js?v=5.6.0`,
            wasmURI: `${libraryLocation}scandit-engine-sdk.wasm?v=5.6.0`,
        };
    }
        ...
        function verifyResponseData(responseData) {
            ...
            if (typeof crypto?.subtle?.digest === "function") {
                crypto.subtle
                    ...
                    // istanbul ignore if
                    if (hashString !== "cbda3ec8ee7bfb9f57b0a8c9977e5e06734a551978620a0852aba6abbb9525b1") {
                        console.error(`The Scandit SDK Engine library WASM file found at ${wasmURI} seems invalid: ` +
                            ...
                            `expected: ${"cbda3ec8ee7bfb9f57b0a8c9977e5e06734a551978620a0852aba6abbb9525b1"}). ` +
                            `Please ensure the correct Scandit SDK Engine file (with correct version) is retrieved.`);
                    }
                })
                    .catch(
    ...
    onmessage = (e) => {
        ...
        switch (data.type) {
            case "load-library":
                ...
                engineInstance.loadLibrary(data.deviceId, data.libraryLocation, data.path, data.preload, data.delayedRegistration, data.highEndBlurryRecognition, data.licenseKey, data.deviceModelName);
                break;
            case "scan-settings":
            ...
            case "camera-properties":
                ...
                break;
            case "device-name":
                engineInstance.setDeviceName(data.deviceName);
                break;
            case "reset":
